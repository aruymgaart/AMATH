...
try:
	...
	print("* tensorFiles : could not find path.py")

try: import baseObject as base
except: base = None

try: import dataUtilities as du
except: du = None
...
def tnsrFile2numpy(fname):
	...
	try:
		...
		base.err(["could not OS-expand (ENV)", fname])
		return np.array([])
	...
	if fname.find(".npz") > -1:
		try:	
			...
			base.err(["could not find .npz", fname])
			return np.array([])
