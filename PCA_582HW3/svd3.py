...
coords1 = pickle.load(open('data/cam1_3_coords.dict','rb'))[0]
coords2 = pickle.load(open('data/cam2_3_coords.dict','rb'))[0]
coords3 = pickle.load(open('data/cam3_3_coords.dict','rb'))[0]
...
x1,p1 = removeLeadingStationary(x1)
x2,p2 = removeLeadingStationary(x2)
x3,p3 = removeLeadingStationary(x3)
print(p1,p2,p3)
y1 = y1[p1:len(y1)]
y2 = y2[p2:len(y2)]
y3 = y3[p3:len(y3)]
...
plotTrendMatrix(np.array([ x1,y1,x2,y2,x3,y3 ]), cNames, ttl='Exp. $3$ aligned coordinate input')
x1, y1 = x1/np.max(y1), y1/np.max(y1)
x2, y2 = x2/np.max(y2), y2/np.max(y2)
x3, y3 = x3/np.max(x3), y3/np.max(x3)
plotTrendMatrix(np.array([ x1,y1,x2,y2,x3,y3 ]), cNames, ttl='Exp. $3$ aligned normalized coordinate input')
#exit()
...
ax[0].set_title('Exp. $3$ aligned normalized coordinate input')
ax[0].plot(x1, label='x1'), ax[0].plot(y1, label='y1'), ax[0].legend()
ax[1].plot(x2, label='x2'), ax[1].plot(y2, label='y2'), ax[1].legend()
ax[2].plot(x3, label='x3'), ax[2].plot(y3, label='y3'), ax[2].legend()
ax[2].set_xlabel("Frame nr.")
plt.legend()
plt.savefig('images/exp3_NONaligned_coordData.png', figsize=(8, 6), dpi=300, bbox_inches='tight')
plt.show()
plt.clf()
...
plt.subplots_adjust(wspace=0, hspace=0)
ax[0].set_title("Exp. $3$ Principal components")
...
for k in range(4): 
	ax[k].legend()
	ax[k].set_yticks([0])
plt.savefig('images/exp3_principal_components.png', figsize=(8, 6), dpi=300, bbox_inches='tight')
plt.legend()
plt.show()
plt.clf()
